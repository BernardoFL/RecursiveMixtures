...
After you change the experiment or plotting code, run `python metastability_experiment.py` (optional `--n-data`, `--k`) and **commit the updated PDF** if you want the repo snapshot to stay aligned. This script has no `--n-data-list` mode; overlay sweeps over several **n** (`paw_hk_overlay_n{n}.pdf`) are generated by [`bootstrap_experiment.py`](bootstrap_experiment.py) (`--study paw`); see [`BOOTSTRAP_EXPERIMENTS.md`](BOOTSTRAP_EXPERIMENTS.md).
